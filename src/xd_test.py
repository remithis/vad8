...
def test(model, testdataloader, maxlen, class_llm_feats, gt, gtsegments, gtlabels, device):
    ...
    with torch.no_grad():
        ...
        # 准备类别原型 (Class Prototypes)
        # 将 7 个类别的 4096 维 LLM 特征投影到 512 维并归一化
        T_class = model.text_proj(class_llm_feats) 
        T_class_norm = F.normalize(T_class, p=2, dim=-1) # [7, 512]
        ...
        for i, item in enumerate(testdataloader):
            ...
            length = item[2]
            llm_video_feat = item[3].to(device) # [1, 4096]
            ...
            # 视频分块处理
            num_chunks = int(length / maxlen) + 1
...
if __name__ == '__main__':
    ...
    # 加载离线 LLM 类别特征 [7, 4096]
    class_llm_feats = torch.load("xd_class_llm_feats.pt").to(device)

    # 特征路径 (指向你提取的视频向量目录)
    llm_dir = "/home/xuchen/Project/VadCLIP-main-yxl/VadCLIP-new/xd_test_embeddings"
    ...
    test(model, test_loader, args.visual_length, class_llm_feats, gt, gtsegments, gtlabels, device)